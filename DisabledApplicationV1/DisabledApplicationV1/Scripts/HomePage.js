    ...
    for (var i = 0; i < result.length; i++) {
     ...
     '<div class="box box-widget" style="border:none; position: relative;     margin-bottom: 20px;" >' +
       ...
       '<div class="box-body" style="border-top-left-radius: 0; border-top-right-radius: 0; border-bottom-right-radius: 3px; border-bottom-left-radius: 3px; padding: 10px;" >' +
                   ...
                   result[i][1] + '</p>' +
       '<div class="row"><div class="span4"><button type="button" id="commentButton' + i + '"' + 'class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal" data-postId=' + result[i][0] + ' style="background:#f4f4f4; color:#444; margin-left: 5px; margin-bottom: 5px; border-color:#ddd;" onclick="myf(' + result[i][0] + ')">' +
       '<i class="fa fa-comments" style="display: inline-block; font: normal normal normal 14px/1 FontAwesome;     font-size: inherit; text-rendering: auto;    -webkit-font-smoothing: antialiased;"></i>' +
        ...
        '<button type="button" id="reportButton' + i + '"' + 'class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal" data-postId=' + result[i][0] + ' style="background:#f4f4f4; color:#444; margin-left: 5px; margin-bottom: 5px; border-color:#ddd;" onclick="reportFunction(' + result[i][0] + "," + i + ')">' +
     '<i class="fa fa-exclamation-triangle" style="display: inline-block; font: normal normal normal 14px/1 FontAwesome;     font-size: inherit; text-rendering: auto;    -webkit-font-smoothing: antialiased;"></i>' +
      report + '</button>' +
      '<button type="button" id="helpedButton' + i + '"' + 'class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal" data-postId=' + result[i][0] + ' style="background:#f4f4f4; color:#444; margin-left: 5px; margin-bottom: 5px; border-color:#ddd;" onclick="markFunction(' + result[i][0] + "," + i + ')">' +
     '<i class="fa fa-check" style="display: inline-block; font: normal normal normal 14px/1 FontAwesome;     font-size: inherit; text-rendering: auto;    -webkit-font-smoothing: antialiased;"></i>' +
      markPost + '</button></div>' +
      '<div class="span4"><span class="pull-right text-muted" style="float: right!important; color:#777;">' + result[i][4] + helperNumber + result[i][5] + reports + result[i][6] + helps + '</span>' +
      '</div></div></div></div></div>');
...
function unclickableAddList(postID, buttonId, userId) {
 ...
 .done(function (result) {
  // result = result.MessageList;
   if (result.Status == false && result.StatusCode == "War101") {
